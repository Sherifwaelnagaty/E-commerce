...
class Pages extends Controller
{
    ...
    public function AddProducts()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            move_uploaded_file($_FILES['product_image']['tmp_name'],$Profile_Picture);
            if ($_FILES["product_image"]["size"] > 5000) {
                echo "Sorry, your file is too large.";
            }if(!filter_var($_POST['productname'], FILTER_VALIDATE_STRING)===0){
                echo "Please enter a product name.";
            }if(!filter_var($_POST['productprice'], FILTER_VALIDATE_INT)===0){
                echo "Please enter a product price.";
            }if(!filter_var($_POST['producttype'], FILTER_VALIDATE_STRING)===0){
                echo "Please enter a product type.";
            }if(!filter_var($_POST['productID'], FILTER_VALIDATE_INT)===0){
                echo "Please enter a product ID.";
            }
            else{
            $registerModel->setproduct_name(trim($_POST['productname']));
            $registerModel->setproduct_image(trim($Profile_Picture));
